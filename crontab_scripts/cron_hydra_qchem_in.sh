...
if [ $(ls $XYZ_FILEPATH 2>/dev/null | wc -l) -eq 0 ]; then
  ...
  do
    ...
    MOL_NAME=${filename%.*}
    python ~/Pipeline/abin_launcher/abin_launcher.py -p qchem -i ~/Q-CHEM/qchem_queue/${filename} -o ~/Q-CHEM/ -ow -cfg ~/Q-CHEM/qchem_queue/${MOL_NAME}_config.yml > ${ABIN_LOGS}/$(date +"%Y%m%d_%H%M%S")_stdout.log
